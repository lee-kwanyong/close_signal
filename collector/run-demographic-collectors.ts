...
type CollectorTask = {
  key: "population" | "household" | "living-population";
  file: string;
};
...
const ALL_TASKS: CollectorTask[] = [
  {
    key: "population",
    file: "collector/population-region-collector.ts",
  },
  {
    key: "household",
    file: "collector/household-region-collector.ts",
  },
  {
    key: "living-population",
    file: "collector/living-population-region-collector.ts",
  },
];
...
function parseCollectorFilter(): Set<string> | null {
  const raw =
    ...
    process.env.DEMOGRAPHIC_COLLECTOR_KEYS ||
    "";
...
async function main() {
  ...
  if (tasks.length === 0) {
    throw new Error("No demographic collectors selected.");
  }
