...
type CollectorTask = {
  key: "competition" | "spending" | "rent" | "search-trend";
  file: string;
};
...
const ALL_TASKS: CollectorTask[] = [
  {
    key: "competition",
    file: "collector/competition-region-category-collector.ts",
  },
  {
    key: "spending",
    file: "collector/spending-region-category-collector.ts",
  },
  {
    key: "rent",
    file: "collector/rent-region-collector.ts",
  },
  {
    key: "search-trend",
    file: "collector/search-trend-region-category-collector.ts",
  },
];
...
function parseCollectorFilter(): Set<string> | null {
  const raw =
    ...
    process.env.MARKET_COLLECTOR_KEYS ||
    "";
...
async function main() {
  ...
  if (tasks.length === 0) {
    throw new Error("No market collectors selected.");
  }
