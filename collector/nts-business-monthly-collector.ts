...
type NormalizedNtsRow = {
  base_month: string;
  region_code: string;
  region_name: string | null;
  category_id: number;
  category_name: string | null;
  operating_count: number | null;
...
const SOURCE_KEY = process.env.CLOSE_SIGNAL_SOURCE_KEY || "nts_business_monthly";
const FILE_PATH = process.env.NTS_MONTHLY_FILE_PATH || "";
const FILE_URL = process.env.NTS_MONTHLY_FILE_URL || "";
const FILE_ENCODING = (process.env.NTS_MONTHLY_FILE_ENCODING || "utf-8").toLowerCase();
const DEFAULT_BASE_MONTH = process.env.NTS_BASE_MONTH || "";
...
if (!FILE_PATH && !FILE_URL) {
  throw new Error("NTS_MONTHLY_FILE_PATH or NTS_MONTHLY_FILE_URL is required.");
}
  ...
  try {
    return await fetch(url, {
      ...
      headers: {
        ...
        "User-Agent": "close-signal-nts-monthly-collector/1.0",
      },
...
function resolveBaseMonth(row: JsonRecord, sourceLabel: string): string {
  ...
  if (!fallback) {
    throw new Error(
      "Failed to resolve base month. Set NTS_BASE_MONTH=YYYY-MM-01 or include a month column in the file.",
    );
  }
...
function resolveOperatingCount(row: JsonRecord): number | null {
  return toInt(
    getFirst(row, [
      "operating_count",
      "운영중",
...
function normalizeRow(row: JsonRecord, sourceLabel: string): NormalizedNtsRow | null {
  ...
  const operatingCount = resolveOperatingCount(row);

  if (!regionCode || categoryId === null || operatingCount === null) {
    return null;
  }
  ...
  return {
    ...
    close_count: resolveCloseCount(row),
    raw_payload: row,
...
async function tryBeginRun(sourceId: number, sourceLabel: string): Promise<number | null> {
  try {
    ...
    console.warn(`[begin_source_run skipped] ${error instanceof Error ? error.message : String(error)}`);
    return null;
  }
}
...
async function tryFinishRun(
  ...
  try {
    ...
    console.warn(`[finish_source_run skipped] ${error instanceof Error ? error.message : String(error)}`);
  }
}

async function upsertNormalizedRows(rows: NormalizedNtsRow[]): Promise<number> {
  let total = 0;

  for (const batch of chunk(rows, UPSERT_BATCH_SIZE)) {
    const { data, error } = await supabase.rpc("upsert_normalized_nts_business_monthly", {
      p_rows: batch,
    });

    if (error) {
      throw new Error(`upsert_normalized_nts_business_monthly failed: ${error.message}`);
    }
...
async function main() {
  ...
  const normalizedRows = parsedRows
    ...
    .filter((row): row is NormalizedNtsRow => Boolean(row));

  if (normalizedRows.length === 0) {
    const headerPreview = parsedRows[0] ? Object.keys(parsedRows[0]) : [];
    throw new Error(
      `No valid rows were normalized from the NTS monthly file. headers=${JSON.stringify(headerPreview)}`,
    );
  }
